...
## Supabase waitlist_signups table + name field on waitlist forms (2026-07-19)

- **Decision**: Added a `waitlist_signups` Supabase table (`supabase/migrations/20260719000000_create_waitlist_signups.sql`: `id`, `name`, `email unique`, `source`, `created_at`; RLS enabled with **no** policies — only the service-role key, used server-side, can read/write it) as a record store **alongside** the existing ConvertKit integration, not a replacement. `netlify/functions/subscribe.ts` now validates `name` (reusing `validateName` from `src/utils/validation.ts`, newly exported — 2-100 chars) in addition to `email`, calls ConvertKit first (unchanged, still determines the HTTP response), then best-effort upserts `{name, email}` into Supabase via a new `netlify/functions/utils/supabaseClient.ts` helper (`getSupabaseClient()` returns `null` when `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` are unset, so the Supabase write silently no-ops until an operator configures those Netlify env vars — documented in `.env.example`). A Supabase write failure is logged but never fails the client-facing request.
- **Frontend**: All three waitlist UIs now collect a required Name field alongside email: the shared `useWaitlistSubscribe` hook (used by the `EmailCapture` section and `WaitlistModal`) and the standalone `ui/EmailCapture` component (which has its own local state, not the hook). `WaitlistModal`'s initial-focus ref was renamed `emailInputRef` → `nameInputRef` and now points at the name input (first field in tab order).
- **Rationale**: User asked to "set the database up to collect users names and emails." Confirmed via AskUserQuestion: Supabase supplements ConvertKit rather than replacing it (lowest risk, keeps existing email automation), and the Name field is required (not optional) on all three form variants.
- **email uniqueness**: constraint is a plain `unique` on the `email` column (not an expression index on `lower(email)`) specifically so `supabase-js`'s `.upsert(..., { onConflict: 'email' })` can target it — Postgres `ON CONFLICT` needs an arbiter matching the literal column list, which an expression index can't satisfy. This is safe because every caller (all 3 form variants, the Netlify function) already lowercases email before it's sent/stored.
- **Not yet done (needs a human)**: The migration SQL has not been applied to the live Supabase project — this session's `supabase` MCP tool requires an interactive OAuth flow (`claude mcp` / `/mcp`) that can't run non-interactively, and is additionally configured `read_only=true` (see entry below) so it couldn't run DDL even if authenticated. The repo owner needs to either run the migration file via the Supabase SQL editor/CLI, or authenticate the MCP server and (temporarily) switch it to a writable config. `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` also still need to be set as Netlify build/runtime env vars — until then `netlify/functions/subscribe.ts` skips the Supabase write and behaves exactly as before (ConvertKit-only).
- **Tests**: Added `netlify/functions/subscribe.test.ts` (first test coverage for this previously-untested file — 7 tests covering name/email validation and the Supabase best-effort path) and `src/utils/validation.test.ts` (new `validateName` export). Updated the 3 waitlist form test files plus `CTA.test.tsx` (renders `WaitlistModal`) and `tests/e2e/landing-page.spec.ts` for the new required Name field. Full suite: 1891 tests passing, `tsc -b`, `eslint .`, and `npm run build` all clean; bundle size unaffected (102.7 kB gzip main JS, well under the 150 kB budget) since `@supabase/supabase-js` is only imported server-side in the Netlify function, never in client code.
- **Alternatives considered**: expression unique index on `lower(email)` (rejected — breaks `onConflict` upsert targeting as above); making Supabase the sole waitlist backend (rejected per user's explicit choice — keeps ConvertKit's email delivery/confirmation flow intact); optional Name field (rejected per user's explicit choice).
