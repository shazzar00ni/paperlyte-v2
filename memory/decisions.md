...
## Node engine enforcement + commitlint drift (2026-07-07)

- **Decision**: Added `engines.node` (`"^20.19.0 || ^22.13.0 || >=24"`, matching eslint@10/@eslint/js@10's real supported set exactly), `.nvmrc`/`.node-version` pinned to `20.19.0`, `netlify.toml` `NODE_VERSION` pinned to `20.19.0` (was bare `"20"`), and `engine-strict=true` + `min-release-age=7` in `.npmrc`. PR #1289, branch `claude/eslint-missing-dependency-x1o88v`.
- **Rationale**: Issue #1192 (`npm run lint` fails locally with `Cannot find package '@eslint/js'`) was only symptom-patched by PR #1243 (pinning `@eslint/js`/`eslint` to concrete versions) and administratively closed 2026-07-04, but CodeRabbit's issue-enrichment analysis on #1192 correctly identified the real root cause was never fixed: no `engines` field, no `.nvmrc`, and `netlify.toml`'s bare `NODE_VERSION = "20"` can resolve below eslint 10's floor. This PR implements that root-cause fix.
- **Fallout discovered via CI (important)**: turning on `engine-strict=true` immediately broke every CI job with `EBADENGINE`, because `@commitlint/cli` had silently drifted to `^21.1.0` via a Dependabot grouped bump (commit `f31765b`, PR #1199) — `@commitlint/cli@21` requires Node `>=22.12.0`, directly contradicting the existing "Tooling / Commit Convention" decision below (stay on commitlint v20 until repo's Node baseline is raised). This had been silently tolerated because without `engine-strict`, npm only warns. **Reverted `@commitlint/cli` to `^20.5.3`** to match `@commitlint/config-conventional` and restore CI compatibility. Lesson: `engine-strict` is a good idea, but only safe to enable after confirming the whole devDependency tree's engine ranges are consistent with the pinned Node version — Dependabot grouped bumps can silently violate that over time.
- **Bot findings applied**: an automated Gitar review correctly flagged that the initial `engines.node: ">=20.19.0"` was broader than eslint's real support (would let Node 21.x/23.x through, reproducing the exact bug this PR fixes) — narrowed to match eslint's exact range. A Semgrep finding (`npm-missing-minimum-release-age`) recommended `min-release-age=7` in `.npmrc` for supply-chain hardening — added (no-op on the repo's current npm 10.x runners; takes effect once npm is upgraded past 11.10).
- **Alternatives considered**: raising the whole repo's Node baseline to >=22.12 to keep commitlint on v21 (rejected — much larger blast radius across CI/Netlify/docs for an unrelated dev-tooling version bump; out of scope for this fix).
