...
export default defineConfig({
  ...
  // because the Vite plugin only supports Vite 4.x-6.x (project uses Vite 8)
  // Rollup plugin works with Vite 8 since Vite's rolldown is compatible with Rollup plugins
  plugins: [
    react(),
    cspPlugin(),
  ...
  build: {
    ...
    cssCodeSplit: true,
    // Use oxc for faster minification (default in Vite 8 via rolldown)
    minify: 'oxc',
