...
**Important**: This changelog automation requires proper PR labeling to work effectively. PRs should be labeled with `feature`, `bug`, `documentation`, or `chore` to appear in the appropriate sections. Unlabeled PRs will still appear in the changelog but won't be categorized. Configure PR auto-labeling (see Priority 4) to automate this process.
...
5. `codelytv/pr-size-labeler@v1.10.1` → Use commit SHA
   - Has access to `GITHUB_TOKEN` for labeling PRs

**Artifact Action Versions**:
- Use consistent versions across upload/download artifacts (prefer `@v6` or stable `@v4`)
- Example issue: `upload-artifact@v6` with `download-artifact@v7` creates version mismatch
- Current CI workflow needs fixing: lines 92 and 119 use `@v7` while uploads use `@v6`
