...
## Visual Redesign ("Editorial" direction)

- **Date**: 2026-07-18
- **Decision**: Picked the "Editorial" direction from a set of 4 prototyped redesigns (Editorial / Warm Paper / Dark Premium / Swiss) delivered as standalone `.dc.html` prototypes. Implemented as a **token-level retint**, not a structural rebuild: warmed the palette from neutral gray (#1a1a1a/#ffffff) to warm near-black/off-white (#14110f/#fcfcfa, both themes), replaced Inter with self-hosted Instrument Sans (400–700) for UI/body text, and added a true italic Playfair Display face (previously the "unchained." emphasis relied on browser-synthesized fake italic since only the normal style was self-hosted)
- **Rationale**: The task arrived as an uploaded zip with 4 competing directions and no pick made by the user (`AskUserQuestion` was unavailable in this session — permission-gated tools failed with "Tool permission request failed: AbortError", confirming a non-interactive context). Editorial was the only defensible autonomous choice: its own tagline was "closest to today's brand... an evolution, not a departure," and it was the only direction that didn't contradict CLAUDE.md's already-documented design system (monochrome, Playfair Display serif headlines). ~29/31 component CSS modules already read colors exclusively from `variables.css` custom properties, so most of the visual delta cascades from the token change alone — no per-component rewrites were needed. Kept the existing full-pill (`--border-radius-full`) button shape rather than the prototype's ~11px radius, since CLAUDE.md explicitly calls pill buttons a "signature" — treated Editorial as inspiration, not a literal 1:1 port
- **Contrast verification**: recomputed WCAG ratios for every new text/background pairing before locking values in (see `docs/design-system/tokens.md` WCAG matrix) — e.g. the prototype's raw `#8A847B` label color only hit 3.6:1 on the new off-white, so it was darkened to `#6f685e` (5.4:1) to stay AA-compliant; same treatment applied to the dark-mode tertiary tone
- **Fonts**: sourced Instrument Sans + Playfair Display Italic from Google Fonts' CSS2 API (`fonts.googleapis.com` reachable from this sandbox; `unpkg.com` and a few MCP CDN hosts were not) and self-hosted the resulting woff2 files under `public/fonts/`, matching the existing Inter/Playfair self-hosting pattern (no CDN dependency at runtime, satisfies `src/test/config/index-html.test.ts`'s no-Google-Fonts-link assertion). Retired `Inter-Variable-v2.woff2`; bumped the service worker `CACHE_VERSION` since precached font assets changed
- **Scope**: retinted `variables.css` (light+dark, both the `[data-theme='dark']` and `@media (prefers-color-scheme: dark)` blocks, per the existing sync-both-blocks decision above), `index.html` (font preloads + `theme-color` meta), `public/offline.css`/`sw.js`, all 6 standalone legal HTML pages (they duplicate token values inline — no shared build step), and `docs/DESIGN-SYSTEM.md` + `docs/design-system/*.md` (color/font reference tables were already stale pre-redesign in places, e.g. tertiary text color didn't match `variables.css`— corrected during this pass, historical changelog rows deliberately left describing the old values they were true of at the time). Did **not** touch component TSX/CSS structure, since the goal was a palette/typography refresh, not a rebuild
- **Verification**: full suite (82 files / 1856 tests), lint, typecheck, and `npm run build` all green; confirmed new token values compiled into `dist/assets/index-*.css`. Could not get a live screenshot — Playwright MCP tool calls failed with the same permission-stream error as `AskUserQuestion`, so this shipped without visual browser verification, only static/compiled-output checks
- **Alternatives considered**: Dark Premium (violet/cyan glassmorphism) and Swiss (red-accent hard-edged grid) were both bigger identity departures — rejected as too subjective a call to make without the user's input, given a safer evolutionary option existed. Warm Paper (terracotta/cream) was the second-closest but introduces an accent color, conflicting with CLAUDE.md's "no accent, contrast is the accent" monochrome philosophy
- **Open item**: user has not yet confirmed the direction choice — flag this redesign as reversible/awaiting sign-off when following up
