...
- **Workflow permissions:** top-level `permissions: {}` (deny-all), with minimum permissions granted at the **job level**. `scorecard.yml` is the documented exception (uses `permissions: read-all` per OSSF guidance).
