...
5. **`@ts-ignore` and `@ts-expect-error` without explanation**: These directives must include a comment on the same line explaining the specific compiler limitation being worked around and a link to the relevant issue if applicable. Bare suppressions are not acceptable.
