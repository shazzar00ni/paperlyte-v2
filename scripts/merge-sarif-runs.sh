...
# =============================================================================
{
  ...
  runs: [{
    ...
    # that were generated by an older Codacy fingerprinting algorithm.
    # Codacy now uses a "hex:version" format (e.g. "9d8c1cf6a28255f9:1").
    #
    # Legacy fingerprints are bare lowercase hex strings of 31–32 characters:
    # - 32 chars: standard MD5 hex digest
    # - 31 chars: MD5 hex digest with a leading zero stripped (integer serialisation)
    #
    # Keeping stale MD5 values alongside new-format ones causes GitHub Code
    # Scanning to emit "inconsistent fingerprint" warnings because it calculates
    # its own fingerprint and finds the SARIF value does not match. Removing
    # them lets GitHub recompute clean, consistent fingerprints on each upload.
    results: [
      .runs[].results // [] | .[] |
      if .partialFingerprints then
        .partialFingerprints |= with_entries(
          select(.value | test("^[0-9a-f]{31,32}$") | not)
        ) |
