...
- **Summary:** An automated repository-wide audit of 310 unmerged branches confirms the following systemic regressions.
...
| Missing `gitVersionControl.md` | 127   | 🟠 High     | Core Git workflow documentation.                                         |
| Missing `review.md`            | 127   | 🟡 Medium   | AI PR reviewer instructions.                                             |
...
### Manual PR Review Feedback (2026-05-24)

#### ⚠️ Critical Alert: Widespread Infrastructure Regressions
Manual inspection of branches marked as 'Ready' (passed file-existence audit) revealed significant **hidden regressions** in infrastructure and configuration files. These branches appear to have been based on a severely outdated or corrupted version of `main`.

**Affected Infrastructure:**
- **`vite.config.ts`**: Loss of `fontPreloadPlugin`, `cspPlugin` enhancements, and Vite 7/8 compatibility fixes.
- **`vercel.json`**: Loss of critical security headers (X-Frame-Options, CSP, HSTS), caching rules, and SPA rewrites.
- **`vitest.config.ts`**: Loss of CI-optimized timeouts, JUnit reporter configurations, and modern coverage providers.
- **`src/utils/navigation.ts`**: Reversion of `scrollToSection` improvements (retry logic for lazy chunks, `prefers-reduced-motion` support).

---

#### PR: `claude/fix-open-redirect-TX551`
- **Status:** Changes Requested (Critical Regression)
- **Feedback:** While the security hardening for `isSafeUrl` and `safeNavigate` is conceptually sound, this branch introduces massive regressions in `vite.config.ts`, `vercel.json`, and `vitest.config.ts`. It also deletes the `safeNavigateExternal` utility.
- **Action:** Rebase on current `main` and restore all infrastructure files.

#### PR: `claude/tree-shake-font-awesome-cK85j`
- **Status:** Changes Requested (Critical Regression)
- **Feedback:** Similar to above, this branch reverts critical project configurations in `vite.config.ts` and `vercel.json`. Additionally, it removes `data-icon` attributes from the `Icon` component, which likely breaks existing unit and E2E tests that rely on these attributes for selectors.
- **Action:** Restore infrastructure files and maintain `data-icon` attributes for test compatibility.

#### PR: `claude/accessibility-audit-baseline-USu5N`
- **Status:** Changes Requested (Infrastructure Regression)
- **Feedback:** This branch contains good accessibility documentation and minor test fixes, but it unnecessarily reverts `vite.config.ts`, `vercel.json`, and `vitest.config.ts`. It also introduces changes to `tsconfig.app.json` (`baseUrl: "."`) that are not required for the current project structure.
- **Action:** Rebase and remove unrelated configuration reversions.
