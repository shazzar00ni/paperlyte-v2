...
describe('Marketing Plan Document Validation', () => {
  ...
  describe('Budget and Resource Allocation', () => {
    it('should have a defined budget table', () => {
      const budget = sections.get('Marketing Channels & Budget Allocation')
        ?.content || content;
  ...
  describe('Metrics and KPIs', () => {
    ...
    it('should have measurement frequencies for metrics', () => {
      ...
      const frequencies = metrics.match(/\|\s*(Daily|Weekly|Monthly|Quarterly)\s*\|/g);
  ...
  describe('Document Quality', () => {
    ...
    it('should not have excessive line length', () => {
      const longLines = lines.filter(line => 
        line.length > 200 && !line.match(/^\|/) // Exclude tables
      );
  ...
  describe('Actionability', () => {
    it('should have specific, measurable targets', () => {
      ...
      const targets = content.match(/\d+[,\d]*\+?\s*(users|subscribers|%|downloads)/gi);
      expect(targets).toBeTruthy();
      expect(targets!.length).toBeGreaterThan(20);
    });

    it('should include specific action items', () => {
      const actionVerbs = content.match(/\b(Launch|Create|Build|Develop|Write|Run|Execute|Track|Monitor|Analyze|Review)\b/gi);
    ...
    it('should reference specific tools and platforms', () => {
      const tools = content.match(/\b(Product Hunt|Twitter|Reddit|Instagram|LinkedIn|Discord|Slack|ConvertKit|Mailchimp|Google Analytics|Mixpanel|Amplitude)\b/gi);
    ...
    it('should provide concrete examples', () => {
      ...
      const examples = content.match(/Example:|For example|e\.g\.|such as|"[^"]+"/gi);
