...
- **Update (2026-07-18, issue [#1196](https://github.com/shazzar00ni/paperlyte-v2/issues/1196))**: Re-verified the counts against current `package-lock.json` — still exactly 14 LGPL-3.0-or-later `@img/sharp-*` packages, and the `@lhci/cli` → `chrome-launcher` → `rimraf@3.0.2` → `glob@7.2.3` → `inflight@1.0.6` chain is unchanged (`@lhci/cli` is already pinned to its latest release, `0.15.1`, confirmed via `npm view`, so there's no newer version to pull in a fixed `chrome-launcher`). Applied the documentation half of the remediation:
  - Added `.fossa.yml` (repo root) with an explicit `policy.license` allowlist (MIT, Apache-2.0, BSD-2-Clause, BSD-3-Clause, ISC, OFL-1.1, MPL-2.0, LGPL-3.0-or-later) — this only affects a future FOSSA **CLI** run, not the hosted GitHub App check
  - Added a "Build-Time Native Dependencies" section to `docs/3RD-PARTY-SOFTWARE.md` listing all 14 LGPL packages plus the two transitive `MPL-2.0` packages (`lightningcss` via `vite`, `axe-core` via `@lhci/cli`)
  - Created `docs/security/fossa-compliance-assessment.md` (pattern matches `docs/security/socket-risk-assessments.md`) with the full root-cause writeup for both checks and step-by-step **FOSSA dashboard** configuration instructions — dashboard access is required to actually silence the GitHub App checks; the in-repo config alone cannot do it
- **Still open**: the GitHub App checks will keep reporting `failure` until a repo owner with FOSSA dashboard access applies the license-allowlist / quality-threshold change documented in the assessment doc. This is expected, not a regression.
