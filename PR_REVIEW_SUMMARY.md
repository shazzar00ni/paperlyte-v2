...
- **Status:** Mixed (Pending Investigation for ESLint)
- **Summary:** Routine maintenance updates for various dependencies. Bumps ESLint and @eslint/js to version 10 (#441, #440), JSDOM to v28 (#442), and others.
- **Feedback:** Most updates appear safe. However, the ESLint 10 updates (#441, #440) are currently failing CI ("Lint and Type Check") and triggered a "1 configuration not found" warning for `.github/workflows/eslint.yml:eslint`. This needs investigation. Other updates like JSDOM v28 are confirmed compatible.
