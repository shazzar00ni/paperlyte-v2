...
## PR #1314 follow-ups: npm audit overrides + SW precache for `legal.css` (2026-07-21)

- **Decision**: While babysitting PR #1314 (the Issue #1310 CSP fix above) two more issues surfaced and were fixed in the same PR rather than deferred:
  1. A required CI gate (`npm audit` inside the "Lint and Type Check" GitHub Actions job) was failing with 4 vulnerabilities (axios, body-parser, brace-expansion, js-yaml) — confirmed pre-existing on `main` and on the PR's original commit (unrelated to the CSP diff), all in dev-only transitive deps (`wait-on`→axios, `@lhci/cli`→express→body-parser, `eslint`'s `minimatch`→brace-expansion, `@commitlint/cli`/`@lhci/cli`→js-yaml). Bumped the existing `overrides` block (same mechanism as the `undici` precedent above): `axios` `^1.16.0`→`^1.18.1`, `brace-expansion` `^5.0.5`→`^5.0.7`, `js-yaml` `^4.2.0`→`^4.3.0`, added `body-parser": "^1.20.6"` (new entry). All stay on their existing major version. `npm audit` → 0 vulnerabilities; lint/typecheck/build/full test suite (1876 tests) all green after the bump.
  2. Codex's PR review flagged that externalizing the six legal pages' CSS into `public/styles/legal.css` broke offline resilience: `public/sw.js`'s fetch router had no rule matching `/styles/legal.css` (only `/assets/*.css` via `HASHED_ASSET_RE` and `offline.css`/`offline.js` via `OFFLINE_ASSET_RE`), so a previously self-contained inline-styled page would now fall through to an unservable network request when viewed offline. Fixed by adding `/styles/legal.css` to `PRECACHE` and extending `OFFLINE_ASSET_RE` to match it (cache-first, same treatment as the other non-hashed root-level offline assets), and bumping `CACHE_VERSION` `'paperlyte-v2'` → `'paperlyte-v3'` per the file's own "bump on any precached-asset change" convention.
- **Rationale**: Both are small, mechanical fixes directly blocking or regressing this specific PR, matching established repo conventions exactly (overrides mechanism, SW versioning scheme) — not scope creep requiring a separate PR.
- **Precedent**: `code/snyk` "Code test limit reached" and FOSSA License Compliance (14 issues)/Dependency Quality (~63-66 issues) remain accepted pre-existing baseline non-blockers (see 2026-06-21 entries above) — unlike the npm-audit gate, these are external quota/account-level and not fixable via this repo's dependency tree.
