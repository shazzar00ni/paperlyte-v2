...
def main():
    ...
    for item in data.get('blocked', []):
        ...
        branch = item['branch']
        if branch in pr_map:
            pr_num = str(pr_map[branch])

            # Check if we have already commented with this exact content
            existing_comments = run_command(['gh', 'pr', 'view', pr_num, '--json', 'comments'])
            already_commented = False
            if existing_comments:
                try:
                    comments_data = json.loads(existing_comments)
                    for comment in comments_data.get('comments', []):
                        if 'Systemic Regressions Detected' in comment.get('body', ''):
            ...
            if not already_commented:
                ...
                run_command(['gh', 'pr', 'comment', pr_num, '--body', comment_body])
