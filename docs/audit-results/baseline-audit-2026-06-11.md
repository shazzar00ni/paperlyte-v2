...
- ⚠️ First-visit offline support doesn't work: the worker is registered on `window.load` (`main.tsx:69-71`) after the page's hashed JS/CSS loaded uncontrolled, and `PRECACHE` contains `/` but none of the hashed bundles. A user who goes offline after one visit gets the cached shell from `navigateFetch()`, whose bundle requests then fail on the network — a blank page instead of `offline.html`. Precache the build assets (inject the manifest at build time) or fall back to `offline.html` when shell dependencies are unavailable
...
1. **Production-domain mismatch (NEW):** static references use `paperlyte.app` (README, `index.html` canonical/OG tags, `public/robots.txt`, the build-time metadata injection), but **two sources actively push `paperlyte.com` into production**: (a) `scripts/generate-sitemap.cjs:10` hardcodes `https://paperlyte.com`, and (b) the **tracked `.env.production`** sets `VITE_BASE_URL=https://paperlyte.com` and `VITE_OG_IMAGE=https://paperlyte.com/og-image.png` — Vite embeds these and `src/main.tsx:66` calls `updateMetaTags()`, which rewrites the canonical URL, `og:url`, and `og:image` at runtime to the `.com` domain and a `.png` that doesn't exist. Fixing the sitemap alone is not enough — so the generated sitemap points search engines at the wrong domain. **Worse, the sitemap never reaches production at all:** `public/sitemap.xml` is not tracked in git, and the `postbuild` step writes it to `public/` only *after* `vite build` has already copied `public/` into `dist` — so deployments contain no `/sitemap.xml` whatsoever. Fix the ordering (generate in `prebuild` or write directly into `dist`) as well as the domain; the December audit also treated `.com` as authoritative and should be read with that caveat.
...
7. **Dead conversion/navigation controls (NEW):** the footer links to `#pricing` (`Footer.tsx:51`) but `App.tsx` never renders the existing `Pricing` component and no rendered section has that ID — clicking it strands the user at the footer. The CTA section's "Watch the Demo" button (`CTA.tsx:44-49`) only runs `scrollToSection('hero')`; no demo video, component, or route exists. Render the pricing section (or drop the link) and relabel/remove the demo button until a demo exists.
8. **Desktop download URLs cannot resolve (carry-over, corrected):** `downloads.ts` expects `Paperlyte-macOS.dmg` / `Paperlyte-Windows.exe` on the latest GitHub release, but `.github/workflows/release.yml` builds and uploads only `paperlyte-<tag>.tar.gz` (+ Sigstore bundle), and no producer for the desktop binaries exists in the repo. The macOS/Windows links will 404 even after a release; the Linux link opens a release containing only the landing-page archive. December's "Download URLs Incomplete" finding therefore remains open for desktop *and* store URLs.
...
7. **Dead controls** — footer `#pricing` link targets an unrendered section; "Watch the Demo" CTA just scrolls to hero (no demo exists)
8. **Runtime metadata domain override** — tracked `.env.production` points canonical/OG tags at `paperlyte.com` and a nonexistent `og-image.png` (part of the domain-mismatch finding)
...
- [ ] Fix the `.env.production` metadata override: `VITE_BASE_URL`/`VITE_OG_IMAGE` point at `paperlyte.com` and a nonexistent `og-image.png`; align with the canonical domain and real image assets
- [ ] Fix dead controls: render the Pricing section or remove the footer `#pricing` link; relabel/remove the "Watch the Demo" CTA until a demo exists
