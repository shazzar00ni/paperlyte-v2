...
for branch in $branches; do
    ...
    npmrc=$(git ls-tree -r $branch --name-only | grep -Fxq ".npmrc" && echo "YES" || echo "NO")
    roadmap=$(git ls-tree -r $branch --name-only | grep -Fxq "docs/ROADMAP.md" && echo "YES" || echo "NO")
    gvc=$(git ls-tree -r $branch --name-only | grep -Fxq "gitVersionControl.md" && echo "YES" || echo "NO")
    review=$(git ls-tree -r $branch --name-only | grep -Fxq "review.md" && echo "YES" || echo "NO")

    # Check for security helpers in src/utils/navigation.ts
    has_helpers="NO"
    if git ls-tree -r $branch --name-only | grep -Fxq "src/utils/navigation.ts"; then
